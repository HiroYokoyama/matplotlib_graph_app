...
3D graphing module for HYGrapher with cross-platform scrolling and project I/O.
"""
...
class GraphApp(BASE_CLASS):
    def __init__(self):
        ...
        self.plot_button = ttk.Button(settings_container, text="Plot / Update 3D Graph", command=self.plot_graph)
        ...
        self.create_scrollable_tab(self.settings_notebook, "3D View / Advanced", self.create_advanced_tab)
        ...
        # Right Panel (3D Graph Preview)
        right_panel = ttk.LabelFrame(content_frame, text="3D Graph Preview")
    ...
    def create_all_tk_variables(self):
        ...
        self.tick_fontsize_var = tk.DoubleVar(value=12.0)
    ...
    def setup_menu_bar(self):
        ...
        file_menu.add_separator()
        file_menu.add_command(label="Open in 2D Mode...", command=self.open_in_2d_mode)
        file_menu.add_separator()
    ...
    def create_basic_settings_tab(self, frame):
        ...
        ttk.Label(top_frame, text="3D Plot Title:").grid(row=0, column=0, padx=3, pady=2, sticky=tk.W)
        self.title_entry = ttk.Entry(top_frame, textvariable=self.title_var, width=25)
        self.title_entry.grid(row=0, column=1, columnspan=3, padx=3, pady=2, sticky=tk.EW)

        ttk.Label(top_frame, text="3D Plot Type:").grid(row=1, column=0, padx=3, pady=2, sticky=tk.W)
        self.plot_type_combo = ttk.Combobox(
            top_frame, textvariable=self.plot_type_var,
            values=["surface", "wireframe", "scatter3d", "line3d", "contour3d"], state='readonly', width=22
        )
        self.plot_type_combo.grid(row=1, column=1, columnspan=3, padx=3, pady=2, sticky=tk.EW)

        # Axes selections
        axes_frame = ttk.LabelFrame(frame, text="3D Coordinates Selection")
        axes_frame.pack(fill=tk.X, padx=2, pady=5)

        ttk.Label(axes_frame, text="X-Axis Data:").grid(row=0, column=0, padx=2, pady=2, sticky=tk.W)
        self.x_axis_combo = ttk.Combobox(axes_frame, textvariable=self.x_axis_var, state='disabled', width=20)
        self.x_axis_combo.grid(row=0, column=1, padx=2, pady=2, sticky=tk.EW)

        ttk.Label(axes_frame, text="Y-Axis Data:").grid(row=1, column=0, padx=2, pady=2, sticky=tk.W)
        self.y_axis_combo = ttk.Combobox(axes_frame, textvariable=self.y_axis_var, state='disabled', width=20)
        self.y_axis_combo.grid(row=1, column=1, padx=2, pady=2, sticky=tk.EW)

        ttk.Label(axes_frame, text="Z-Axis Data (Multi-select):").grid(row=2, column=0, columnspan=2, padx=2, pady=2, sticky=tk.W)
        z_list_frame = ttk.Frame(axes_frame, height=80)
        z_scroll = ttk.Scrollbar(z_list_frame, orient=tk.VERTICAL)
        self.y_listbox = tk.Listbox(z_list_frame, selectmode=tk.MULTIPLE, yscrollcommand=z_scroll.set, exportselection=False, height=4)
        z_scroll.config(command=self.y_listbox.yview)
        z_scroll.pack(side=tk.RIGHT, fill=tk.Y)
        self.y_listbox.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        z_list_frame.grid(row=3, column=0, columnspan=2, padx=2, pady=2, sticky=tk.EW)

    def create_style_settings_tab(self, frame):
        mesh_frame = ttk.LabelFrame(frame, text="3D Mesh & Colormap Options")
        mesh_frame.pack(fill=tk.X, padx=2, pady=2)

        ttk.Label(mesh_frame, text="Mesh Resolution:").grid(row=0, column=0, padx=2, pady=2, sticky=tk.W)
        self.mesh_spin = ttk.Spinbox(mesh_frame, from_=10, to=200, increment=10, textvariable=self.mesh_resolution_var, width=8)
        self.mesh_spin.grid(row=0, column=1, padx=2, pady=2)

        ttk.Label(mesh_frame, text="Colormap:").grid(row=0, column=2, padx=5, pady=2, sticky=tk.W)
        self.cmap_combo = ttk.Combobox(
            mesh_frame, textvariable=self.colormap_var,
            values=['viridis', 'plasma', 'inferno', 'magma', 'coolwarm', 'jet', 'rainbow'], state='readonly', width=12
        )
        self.cmap_combo.grid(row=0, column=3, padx=2, pady=2)
    ...
    def create_axis_ticks_tab(self, frame):
        ttk.Label(frame, text="3D Axis Limits", font=("-weight bold")).grid(row=0, column=0, sticky=tk.W, pady=3)

    def create_spines_tab(self, frame):
        ttk.Label(frame, text="3D Background Colors").pack(anchor=tk.W, pady=2)

    def create_legend_tab(self, frame):
        self.legend_show_check = ttk.Checkbutton(frame, text="Show 3D Legend", variable=self.legend_show_var)
        self.legend_show_check.pack(anchor=tk.W, pady=2)

    def create_advanced_tab(self, frame):
        view_frame = ttk.LabelFrame(frame, text="3D Camera View Angles")
        view_frame.pack(fill=tk.X, padx=2, pady=5)

        ttk.Label(view_frame, text="Elevation (-90° to 90°):").grid(row=0, column=0, padx=2, pady=2, sticky=tk.W)
        self.elev_spin = ttk.Spinbox(view_frame, from_=-90, to=90, increment=5, textvariable=self.view_elev_var, width=8)
        self.elev_spin.grid(row=0, column=1, padx=2, pady=2)

        ttk.Label(view_frame, text="Azimuth (-180° to 180°):").grid(row=0, column=2, padx=5, pady=2, sticky=tk.W)
        self.azim_spin = ttk.Spinbox(view_frame, from_=-180, to=180, increment=5, textvariable=self.view_azim_var, width=8)
        self.azim_spin.grid(row=0, column=3, padx=2, pady=2)
    ...
    def update_plot_options(self):
        ...
        if cols:
            self.x_axis_var.set(cols[0])
            if len(cols) > 1:
                self.y_axis_var.set(cols[1])
            if len(cols) > 2:
                self.y_listbox.select_set(2)

    def get_data_from_sheet(self):
        if not self.sheet or self.df is None:
            return
        try:
            data = self.sheet.get_sheet_data() if hasattr(self.sheet, 'get_sheet_data') else getattr(self.sheet, 'data', None)
            headers = self.sheet.get_headers() if hasattr(self.sheet, 'get_headers') else getattr(self.sheet, 'headers', None)
            if callable(headers):
                headers = headers()
            self.data_mgr.update_from_sheet_data(data, headers)
        except Exception as e:
            print(f"Sheet retrieval error: {e}")
    ...
    def plot_graph(self):
        try:
            ...
            messagebox.showerror("Internal Error", f"Failed to clear 3D graph:\n{e}")
            return
        ...
        z_cols = [self.y_listbox.get(i) for i in self.y_listbox.curselection()]

        if not x_col or not y_col or not z_cols:
            messagebox.showerror("Error", "Please select X, Y, and at least one Z column.")
            return
        ...
        try:
            ...
            for z_c in z_cols:
                ...
                if plot_type in ["surface", "wireframe", "contour3d"]:
                    res = self.mesh_resolution_var.get()
                    ...
                    if plot_type == "surface":
                        self.ax.plot_surface(Xi, Yi, Zi, cmap=self.colormap_var.get(), alpha=0.8)
                    elif plot_type == "wireframe":
                        self.ax.plot_wireframe(Xi, Yi, Zi, rstride=5, cstride=5)
                    elif plot_type == "contour3d":
                        self.ax.contour3D(Xi, Yi, Zi, 20, cmap=self.colormap_var.get())
            ...
            messagebox.showerror("Plot Error", f"Failed to plot 3D graph:\n{e}")

    def save_settings(self):
        file_path = filedialog.asksaveasfilename(
            title="Save 3D Project",
    ...
    def export_graph(self):
        ...
        if file_path:
            self.fig.savefig(file_path, dpi=self.export_dpi_var.get(), bbox_inches='tight')
    ...
    def on_drop(self, event):
        ...
        if files:
            file_path = str(files).strip('{}').strip()
    ...
    def load_project_file(self, file_path):
        try:
            ...
            if 'edited_data' in settings and settings['edited_data']:
                ...
                if self.sheet:
                    self.sheet.destroy()
                self.sheet = Sheet(self.sheet_frame, data=self.df.values.tolist(), headers=self.df.columns.tolist())
                self.sheet.enable_bindings()
                self.sheet.pack(fill=tk.BOTH, expand=True)
                self.update_plot_options()
    ...
    def open_in_2d_mode(self):
        ...
        if self.current_project_path and os.path.exists(self.current_project_path):
            cmd.append(self.current_project_path)
        subprocess.Popen(cmd)

    def show_about(self):
        messagebox.showinfo("About HYGrapher 3D", f"HYGrapher 3D ver. {VERSION}\n3D Data Visualization Tool.")
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        if os.path.exists(file_path):
            app.after(100, lambda: app.load_project_file(file_path))
    app.mainloop()
